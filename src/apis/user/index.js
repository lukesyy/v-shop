import request from '@/utils/http/request';

/**
 * 查看用户资产
 */
export function userAmount(data) {
  return request({
    url: `/user/amount`,
    method: 'get',
    params: data,
  });
}

/**
 * 资金流水
 */
export function userCashLog(data) {
  return request({
    url: `/user/cashLog/v2`,
    method: 'post',
    data,
  });
}

/**
 * 查看用户详情
 */
export function userDetail(data) {
  return request({
    url: `/user/detail`,
    method: 'get',
    params: data,
  });
}

/**
 * 修改用户信息
 */
export function userModify(data) {
  return request({
    url: `/user/modify`,
    method: 'get',
    params: data,
  });
}

/**
 * 退出登录
 */
export function userLoginOut(data) {
  return request({
    url: `/user/loginout`,
    method: 'get',
    params: data,
  });
}

/**
 * 绑定手机号码
 */
export function userMBindMobile(data) {
  return request({
    url: `/user/m/bind-mobile`,
    method: 'post',
    data,
  });
}

/**
 * 用户登录[手机号码]
 */
export function userMLogin(data) {
  return request({
    url: `/user/m/login`,
    method: 'post',
    data,
  });
}

/**
 * 使用手机号码和验证码登录
 */
export function userMLoginMobile(data) {
  return request({
    url: `/channel/user/login`,
    method: 'post',
    data,
  });
}

/**
 * 用户注册[手机号]
 */
export function userMRegister(data) {
  return request({
    url: `/user/m/register`,
    method: 'post',
    data,
  });
}

/**
 * 重设登陆密码
 */
export function userMResetPwd(data) {
  return request({
    url: `/user/m/reset-pwd`,
    method: 'post',
    data,
  });
}

/**
 * 修改登陆密码
 */
export function userModifyPwd(data) {
  return request({
    url: `/user/modify/password`,
    method: 'post',
    data,
  });
}

/**
 * 添加收货地址
 */
export function userShoppingAddressAdd(data) {
  return request({
    url: `/channel/user/address/add`,
    method: 'post',
    data,
  });
}

/**
 * 获取默认收货地址
 */
export function userShoppingAddressDefault(data) {
  return request({
    url: `/channel/user/address/getDefault`,
    method: 'get',
    params: data,
  });
}

/**
 * 删除收货地址
 */
export function userShoppingAddressDelete(data) {
  return request({
    url: `/channel/user/address/remove`,
    method: 'post',
    data,
  });
}

/**
 * 收货地址详情
 */
export function userShoppingAddressDetail(data) {
  return request({
    url: `/user/shipping-address/detail/v2`,
    method: 'get',
    params: data,
  });
}

/**
 * 收货地址列表
 */
export function userShoppingAddressList(data) {
  return request({
    url: `/channel/user/address/list`,
    method: 'post',
    data,
  });
}

/**
 * 修改收货地址
 */
export function userShoppingAddressUpdate(data) {
  return request({
    url: `/channel/user/address/edit`,
    method: 'post',
    data,
  });
}

export default {
  userAmount,
  userCashLog,
  userDetail,
  userModify,
  // 登录/注册
  userLoginOut,
  userMBindMobile,
  userMLogin,
  userMLoginMobile,
  userMRegister,
  userMResetPwd,
  userModifyPwd,
  // 收货地址
  userShoppingAddressAdd,
  userShoppingAddressDefault,
  userShoppingAddressDelete,
  userShoppingAddressDetail,
  userShoppingAddressList,
  userShoppingAddressUpdate,
};
